  ...
  return (
    <div className="h2-page h2-analysis-page">
      ...
      <section className="h2-panel h2-chart-panel">
        ...
        {workspace.series && selectedField ? <EChartsCanvas ariaLabel={`${formatH2FieldLabel(selectedField.name)}时间序列图`} option={createVariableChartOption(workspace.series, selectedField)} /> : <div className="h2-chart-empty"><strong>变量趋势不可用</strong><p>{workspace.seriesError ?? '当前数据源没有返回可绘制序列。'}</p></div>}
      </section>
      ...
      <section className="h2-panel h2-field-dictionary">
        ...
        <div className="h2-table-scroll"><table className="h2-table"><thead><tr><th>中文名称</th><th>字段键</th><th>角色</th><th>单位</th><th>必填</th></tr></thead><tbody>{toH2FieldDictionaryRows(workspace.run.dataset.fields).map((field) => <tr key={field.name}><td><strong>{field.chineseName}</strong></td><td><code>{field.name}</code></td><td>{field.role}</td><td>{field.unit || '—'}</td><td>{field.required ? '是' : '否'}</td></tr>)}</tbody></table></div>
      </section>
