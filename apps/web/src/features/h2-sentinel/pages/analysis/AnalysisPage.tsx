...
type DedicatedView = 'pcc' | 'soc' | 'quota'

const dedicatedViews = [
  { id: 'pcc', label: 'PCC 功率边界线', ariaLabel: 'PCC 实际功率与上网、下网功率边界时间序列图' },
  { id: 'soc', label: 'SOC 目标轨迹', ariaLabel: '储能实际 SOC 与目标 SOC 轨迹时间序列图' },
  { id: 'quota', label: '电量配额', ariaLabel: '当日累计上网、下网电量与配额时间序列图' },
] as const satisfies readonly {
  readonly id: DedicatedView
  ...
  return (
    <div className="h2-page h2-analysis-page">
      ...
      <section className="h2-panel h2-chart-panel">
        <div className="h2-panel__heading"><div><p className="h2-eyebrow">Variable explorer</p><h2>变量趋势</h2></div>{selectedField ? <label className="h2-inline-select"><span className="h2-visually-hidden">选择变量</span><select value={selectedField.name} onChange={(event) => setSelectedVariable(event.currentTarget.value)}>{chartableFields.map((field) => <option key={field.name} value={field.name}>{formatH2FieldLabel(field.name)} · {formatH2FieldUnit(field.name) || '无单位'}</option>)}</select></label> : null}</div>
        {series && selectedField ? <EChartsCanvas ariaLabel={`${formatH2FieldLabel(selectedField.name)}时间序列图`} option={createVariableChartOption(series, selectedField)} /> : <div className="h2-chart-empty"><strong>变量趋势不可用</strong><p>{workspace.seriesError ?? '当前数据源没有返回可绘制序列。'}</p></div>}
      </section>

      <section className="h2-panel h2-chart-panel">
        <div className="h2-panel__heading"><div><p className="h2-eyebrow">Dedicated views</p><h2>专用视图</h2></div><span>时间轴一致 · 可筛选缩放</span></div>
        <div className="h2-view-tabs" role="tablist" aria-label="专用视图切换">
          {dedicatedViews.map((view) => (
            <button
              aria-pressed={dedicatedView === view.id}
              className={dedicatedView === view.id ? 'is-active' : ''}
              key={view.id}
              onClick={() => setDedicatedView(view.id)}
              role="tab"
              type="button"
            >
              {view.label}
        ...
        {series ? (
          dedicatedView === 'pcc' ? (
            <EChartsCanvas ariaLabel="PCC 实际功率与上网、下网功率边界时间序列图" option={createPccChartOption(series)} />
          ) : dedicatedView === 'soc' ? (
            <EChartsCanvas ariaLabel="储能实际 SOC 与目标 SOC 轨迹时间序列图" option={createSocChartOption(series)} />
          ) : quotaAvailable ? (
            <EChartsCanvas ariaLabel="当日累计上网、下网电量与配额时间序列图" option={createQuotaChartOption(series)} />
          ) : (
            <div className="h2-chart-empty" role="status"><strong>电量配额数据暂不可用</strong><p>当前数据源没有返回当日电量配额变量；不会绘制推测曲线。</p></div>
          )
        ) : (
          <div className="h2-chart-empty" role="status"><strong>专用视图不可用</strong><p>{workspace.seriesError ?? '当前数据源没有返回可绘制序列。'}</p></div>
        )}
      </section>
