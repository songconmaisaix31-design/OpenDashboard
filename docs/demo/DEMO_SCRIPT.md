...
Artifact status: `designed`. This script is not runtime or visual evidence.
T4 must satisfy the claim gates before using it as external submission
material.
...
| 10–22s | Open the `order-api` incident. | Target is `degraded`; incident is open with rule `api-error-burst`. | "A fixture-owned transient latch caused this incident; this is not a source-code defect." |
...
| 62–72s | Approve as the demo user. | `approveAction` returns `action_confirmed`; execution mode is `simulated`. | "Approval clears only the fixture latch. It does not restart a real process or repair code." |
| 72–83s | Run recovery verification. | `verifyRecovery` returns `recovered`; target is healthy and the audit includes recovery verification. | "The fixture returns to healthy, and the before-and-after transition is recorded." |
