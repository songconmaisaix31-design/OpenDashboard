...
| ID | Approved wording | Status | Current evidence | Release evidence |
|---|---|---|---|---|
...
| `P0-001` | The candidate runs one deterministic five-phase incident-to-recovery journey for `order-api`. | `implemented` | Verified | T1 golden-path test and T4 integrated Chrome run. |
| `P0-002` | The read-only `api-500-triage` workflow collects redacted HTTP, trace, log, and resource evidence from the fixture. | `implemented` | Verified | T1 evidence test, presentation tests, and T4 visual evidence. |
| `P0-003` | Cordis, LocalOps, Agent Usage Manager, FastAPI Radar, Hardware, Orca, and AgentTeams observations are fixture-backed and visibly mocked; none is a live integration. | `mocked` | Verified | Provenance contract, fixture data, persistent UI badges, and T4 claim review. |
| `P0-004` | A demo user must approve the simulated managed-runtime restart before the fixture action is confirmed. | `implemented` | Verified | T1 invalid-transition/approval tests and T4 approval-state run. |
| `P0-005` | Recovery verification moves the fixture target from degraded to healthy and the incident from open to recovered. | `implemented` | Verified | T1 before/after test and T4 golden-path run. |
| `P0-006` | The local evidence report contains redacted before/after state, provenance, approval, simulated action, verification, audit history, and unverified claims. | `implemented` | Verified | T1 export test and T4 representative report review. |
| `P0-007` | Reset restores the deterministic initial fixture state. | `implemented` | Verified | T1 reset tests and T4 repeated run. |
| `P0-008` | The guided interface keeps fixture/mock provenance, approval, and audit state visible at each relevant phase. | `implemented` | Verified | Presentation tests, desktop screenshots, and T4 visual review. |
